...
// Section management IPC handlers
ipcMain.handle("get-sections", async () => {
  return await database.getSections();
});
...
ipcMain.handle("update-section", async (event, id, section) => {
  return await database.updateSection(id, section);
});
...
// Firebase sync IPC handler
ipcMain.handle("firebase:sync-sections-tables", async (event, data) => {
  const { initializeFirebaseAdmin, setDocument, updateDocument, queryCollection } = require("./firebase/electronIntegration");
  try {
...
ipcMain.handle("firebase:get-sections", async (event) => {
  ...
  try {
    ...
    console.error('Error getting sections from Firebase:', error);
    return { success: false, error: 'Failed to get sections', sections: [] };
  }
});

// Firebase get tables from Firestore
ipcMain.handle("firebase:get-tables", async (event) => {
  const { queryCollection } = require("./firebase/electronIntegration");
  try {
    ...
    console.error('Error getting tables from Firebase:', error);
    return { success: false, error: 'Failed to get tables', tables: [] };
  }
});
